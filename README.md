...
![EyeFocus 界面截图](imgs/screenshot_dark.png)
![EyeFocus 界面截图](imgs/screenshot_light.png)
...
- **工作流程**：双眼依次模式会先测左眼再测右眼。每只眼采用自适应阶梯策略逐步调整视标大小（按“连续答对 3 次升级 / 答错 2 次降级”的规则），并在若干轮检测后判断是否达到稳定等级（即连续多次保持同一等级或在邻近等级间波动收敛）。最终结果基于该收敛判断得到更稳定的视力值。

- **注意**：双眼依次并非简单取两次独立测量的平均值，而是依靠阶梯策略与稳定性判据来获得更可靠的最终等级，适用于存在短暂波动或受测试环境影响的情况。

## 校准与开始前准备（重要）

- 在开始任何测试前，请务必先进入「参数校准」并确认或设置显示器的**对角线尺寸（英寸）**与**当前分辨率（宽 × 高）**。物理像素密度（px/mm）由这两项决定，直接影响视标的物理尺寸换算，校准错误会导致视力结果偏差。

- 建议在设备首次部署或切换显示器时，按实际屏幕规格填写参数；若不确定尺寸请手动测量屏幕对角线后填写。

## 演示视频

项目内已包含录屏演示用于展示软件界面与完整检测流程（位置：`videos/screen_record.mp4`）。

- 屏幕录制（已包含）：`videos/screen_record.mp4` — 包含参数校准、单眼/双眼检测流程与打印示例。
- 真人演示（建议添加）：可添加一份真人操作视频（建议路径 `videos/live_demo.mp4`），用于真实受检者的测试场景展示。

页面（`index.html`）已增加“演示”栏目，展示以上视频。若要替换或添加视频文件，请将文件放入 `videos/` 目录并在 `index.html` 中更新对应文件名或海报。
